...
function initDriverDashboardControls(){
    ...
    const getPermissionAlert = document.getElementById('get-permission-alert');
    ...
    const stoppageAlterCard = document.getElementById('stoppage-alert-card');
    ...
    const btnCancleEnd = document.getElementById('btn-cancle-end');
    ...
    if(navigator.permissions && navigator.permissions.query){
        ...
        }).catch(err=> console.log("Permission API querry fallback",err));
    }
    ...
    if(btnToggleDebug && gpsDebugPanel.gpsDebugPanel){
    ...
    if(btnStart){
        btnStart.addEventListener('click',()=>{
            ...
            btnStart.textContent = "Requestin gfresh gps--";
            if(gpsPermissionAlert) gpsPermissionAlert.classList.remove('d-none');
            if(gpsStatusText){
                gpsStatusText.textContent = "Obtaning fresh gps position";
                gpsStatusText.style.color = "#f59e0b"
            }
            ...
            navigator.geolocation.getCurrentPosition(
                async(pos) => {
                    ...
                    try{
                        const res = await fetch('/api/start-jpurney/',{
                            method:'POST',
                            headers: {
                        ...
                        if(data.success){
                            ...
                            alter(data.error || "Failed to start journey");
                            btnStart.disabled = false;
                            btnStart.textContent = "Start Journey";
                        }
                    }catch(err){
                        console.error("GPS error",err);
                        alter("Network error with server", err.message);
                        btnStart.disabled = false;
                        btnStart.textContent = "Start Journey";
                    }
                },
                (err) =>{
                    ...
                    let errorMsg = "Location req failed";
                    if (err.code === err.PERMISSION_DENIED) {
                        ...
                        errorMsg = "GPS requtest time out";
                        if(gpsStatusText){
    ...
    if(activeTripSection){
        const tripId = activeTripSection.CDATA_SECTION_NODE.tripId;

        const mapElem = document.getElementById('driver-map');
        if(mapElem){
            driverMAp = L.map('driver-map').setView([12.9779,7757244],10);
        ...
        if('geolocation' in navigator){
            ...
            watchId = navigator.geolocation.watchPosition(
                ...
                (err) =>{
                    ...
                    updateGpsSignalUI(false,nunll,err.message);
                },
                watchOptions
            );
