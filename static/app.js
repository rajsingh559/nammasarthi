...
function initPassengerDashboard() {
    ...
    if (searchForm) {
        ...
        window.addEventListener('languageChanged', () => {
            if (lastSearchResults && busCardsContainer) {
                busCardsContainer.innerHTML = '';
                lastSearchResults.forEach(b => {
                    const card = createBusCard(b);
                    busCardsContainer.appendChild(card);
                });
            }
    ...
    function createBusCard(bus) {
        ...
        let statusBadge = `<span class="badge badge-status-live">${t('status_live', '🟢 LIVE')}</span>`;
        if (bus.status === 'DELAYED' || bus.status === 'STOPPED') {
            statusBadge = `<span class="badge badge-status-delayed">${t('status_delayed', '🟠 DELAYED')}</span>`;
        }
        ...
        const delayNotice = bus.stop_reason ? `
            <div class="alert alert-warning" style="margin-top: 0.75rem; padding: 0.75rem; font-size: 0.85rem;">
                <strong>${t('bus_delayed_warning', '⚠️ BUS DELAYED')}</strong><br>${t('reason_label', 'Reason:')} ${bus.stop_reason}
            </div>
        ` : '';
        ...
        let sourceMainText = t('arrival_unavailable', 'Arrival time unavailable');
        let sourceSubtext = t('arrival_unavailable', 'ETA unavailable');
        if (hasSourceEta) {
            const sourceEta = parseInt(bus.eta_source_min, 10);
            if (sourceEta <= 0) {
                sourceMainText = t('reached_your_stop', 'Bus has reached your stop');
                sourceSubtext = t('arriving_now', 'Arriving now');
            } else {
                const clockTime = calculateArrivalTime(sourceEta);
                sourceMainText = clockTime ? `${t('arriving_at', 'ARRIVING AT')} ${clockTime}` : t('arrival_unavailable', 'Arrival time unavailable');
                sourceSubtext = `${sourceEta} ${t('min_from_now', 'min from now')}`;
            }
        }

        // 2. YOUR JOURNEY CALCULATIONS (destination_eta - your_stop_eta)
        let journeyMainText = t('journey_unavailable', 'Journey duration unavailable');
        let journeySubtext = "";
        if (hasSourceEta && hasDestEta) {
            ...
            if (journeyMin < 0) {
                journeyMainText = t('journey_unavailable', 'Journey duration unavailable');
                journeySubtext = "";
            } else if (journeyMin === 0) {
                journeyMainText = `0 min`;
                journeySubtext = t('from_stop_to_dest', 'from your stop to destination');
            } else {
        ...
        let destMainText = t('arrival_unavailable', 'Arrival time unavailable');
        let destSubtext = t('arrival_unavailable', 'ETA unavailable');
        if (hasDestEta) {
            const destEta = parseInt(bus.eta_dest_min, 10);
            if (destEta <= 0) {
                destMainText = t('reached_dest', 'Bus has reached destination');
                destSubtext = t('reached_dest', 'Reached destination');
            } else {
                const clockTime = calculateArrivalTime(destEta);
                destMainText = clockTime ? `${t('reaching_at', 'REACHING AT')} ${clockTime}` : t('arrival_unavailable', 'Arrival time unavailable');
                destSubtext = `${destEta} ${t('min_from_now', 'min from now')}`;
            }
        }
        ...
        div.innerHTML = `
            <div class="bus-card-header">
                <div>
                    ...
                    <p class="bus-route-name">${t('service_type_label', 'Service:')} <strong>${bus.service_type}</strong></p>
                </div>
                <div>${statusBadge}</div>
            </div>

            <div style="font-size: 0.9rem; margin-bottom: 0.5rem;">
                <p><strong>${t('current_stop_label', 'Current Stop:')}</strong> Near ${bus.current_stop_name}</p>
                <p><strong>${t('next_stop_label', 'Next Stop:')}</strong> ${bus.next_stop_name}</p>
                <p><strong>${t('dist_to_your_stop_label', 'Distance to Your Stop:')}</strong> ${bus.dist_to_source_km} km</p>
            </div>

            <div class="eta-timeline">
                <div class="timeline-box timeline-box-stop">
                    <div class="timeline-label">${t('your_stop', '📍 YOUR STOP')}</div>
                ...
                <div class="timeline-box timeline-box-journey">
                    <div class="timeline-label">${t('your_journey', '🚌 YOUR JOURNEY')}</div>
                ...
                <div class="timeline-box timeline-box-dest">
                    <div class="timeline-label">${t('destination', '🏁 DESTINATION')}</div>
            ...
            <button type="button" class="btn btn-primary btn-block btn-view-map" data-trip-id="${bus.trip_id}" data-bus-num="${bus.bus_number}" data-route="${bus.route_name}">
                ${t('btn_view_map', '🗺️ VIEW LIVE MAP')}
            </button>
        `;
